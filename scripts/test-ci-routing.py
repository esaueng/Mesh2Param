...
class TrustedPolicyTests(unittest.TestCase):
    def setUp(self):
        ...
        self.context = {
            "REPOSITORY": self.scope["REPO"],
            "REPOSITORY_ID": str(self.scope["REPO_ID"]),
            "ACTOR": self.scope["ACTOR"],
            "ACTOR_ID": str(self.scope["ACTOR_ID"]),
            "TRIGGERING_ACTOR": self.scope["ACTOR"],
    ...
    def test_policy_precedes_checkout_and_no_secrets(self):
        ...
        self.assertRegex(
            text, r"uses: esaueng/remus/.github/workflows/select-runner.yml@[a-f0-9]{40}"
        )
