...
export function TopBar({vm,actions,onToggleShortcuts}:{vm:WorkspaceViewModel;actions:WorkspaceActions;onToggleShortcuts():void}){const [name,setName]=useState(vm.project.name);useEffect(()=>setName(vm.project.name),[vm.project.name]);return <header className="topbar">
  ...
  <Button className="topbar-action secondary-action" variant="primary" onClick={()=>{actions.setStep("features");void actions.run("reconstruct")}} disabled={!vm.workerReady||!vm.serverWritable||!vm.project.state.source||Boolean(vm.activeJob)}><Sparkles size={16}/>Auto reconstruct</Button>
